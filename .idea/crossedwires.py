def move(direction, distance):
    print("moving %d in direction %s" % (distance, direction))
    global x
    global y
    if direction == "U":
...
def intersectCheck(direction, distance):
    print("checking intersect %d in direction %s" % (distance, direction))
    global x
    global y
    if direction == "U":
        for i in range(distance):
            y += 1
            if (x, y) in posList:
                print("Found intersect at (%d, %d)" % (x, y))
                crossList.append((x, y))
    elif direction == "D":
        for i in range(distance):
            y -= 1
            if (x, y) in posList:
...
for instruction in wireTwo:
    intersectCheck(instruction[0], int(instruction[1:]))

distanceList = []
for item in crossList:
    print(item)
    distanceList.append(item[0] + item[1])

for dist in reversed(sorted(distanceList)):
    print(dist)